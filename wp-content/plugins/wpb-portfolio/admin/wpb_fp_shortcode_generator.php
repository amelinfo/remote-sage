...
function wpb_fp_shortcode_generator_metaboxes_style_settings(){
	...
	$fields = array(
		array(
			'label'	=> __( 'Filtering Script', WPB_FP_TEXTDOMAIN ),
			'desc'	=> __( 'Choose a portfolio filtering script.', WPB_FP_TEXTDOMAIN ),
			'id'	=> $prefix.'filtering_script',
			'type'	=> 'select',
			'options' => array (
				'mixitup' => array (
					'label' => __( 'Mixitup', WPB_FP_TEXTDOMAIN ),
					'value'	=> 'mixitup'
				),
				'isotope' => array (
					'label' => __( 'isotope', WPB_FP_TEXTDOMAIN ),
					'value'	=> 'isotope'
				),
			)
		),
		array(
			'label'		=> __( 'Portfolio Skin', WPB_FP_TEXTDOMAIN ),
			'desc'		=> __( 'Select a skin for portfolio.', WPB_FP_TEXTDOMAIN ),
		...
		array(
			...
			'options' 	=> array (
				'no' => array (
					'label' => __( 'No', WPB_FP_TEXTDOMAIN ),
					'value'	=> 'no'
				),
				'yes' => array (
					'label' => __( 'Yes', WPB_FP_TEXTDOMAIN ),
					'value'	=> 'yes'
				),
			)
		),
		...
		array(
			'label'			=> __( 'Select any default WordPress image size', WPB_FP_TEXTDOMAIN ),
			'desc'			=> __( 'Use this if you are not hard croping images.', WPB_FP_TEXTDOMAIN ),
	...
	*/
	...
	$portfolio_metabox = new WPB_FP_Custom_Add_Meta_Box( 'portfolio_style_settings', __( 'Portfolio Style Options', WPB_FP_TEXTDOMAIN ), apply_filters( 'wpb_fp_shortcode_generator_metabox', $fields, $prefix ), $post_type, 'normal' );
...
function wpb_fp_shortcode_generator_metaboxes_show_shortcode(){
	...
	*/
	...
	$portfolio_metabox = new WPB_FP_Custom_Add_Meta_Box( 'portfolio_show_shortcode', __( 'Portfolio Shortcode', WPB_FP_TEXTDOMAIN ), apply_filters( 'wpb_fp_shortcode_generator_metabox', $fields, $prefix ), $post_type, 'side' );
...
if( !function_exists('wpb_fp_generated_shortcode_function') ):
	function wpb_fp_generated_shortcode_function( $atts ){
		...
		echo do_shortcode( '[wpb-another-portfolio '.$pagination.' '.$filtering.' '.$posts.' '.$orderby.' '.$order.' '.$column.' '.$width.' '.$height.' '.$post_type.' '.$taxonomy.' '.$fp_category.' '.$exclude_tax.' '.$wpb_fp_skin.' '.$img_hard_crop.' '.$img_no_hard_crop_size.' '.$filtering_script.' ]' );
